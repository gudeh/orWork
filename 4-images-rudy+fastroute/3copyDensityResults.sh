...
# Find and copy stage 3 and stage 5 CSV files
find ./reports -type f \( -name "*stg3*.csv" -o -name "*stg5*.csv" \) -exec cp {} "$destination_folder" \;

# Find and copy stage 3 and stage 5 PNG files
find ./reports -type f \( -name "*stg3*.png" -o -name "*stg5*.png" \) -exec cp {} "$destination_folder" \;
